...
RSpec.describe Paghiper::Transaction do
  ...
  context 'with success' do
    before do
      FakeWeb.register_uri(:post, 'https://api.paghiper.com/transaction/create', body: File.join(File.dirname(__FILE__), '..', 'fixtures', 'transaction', 'create_with_success.json'), status: [201, 'CREATED'])
    end
  ...
  context 'with error' do
    before do
      FakeWeb.register_uri(:post, 'https://api.paghiper.com/transaction/create', body: File.join(File.dirname(__FILE__), '..', 'fixtures', 'transaction', 'create_with_error.json'), status: [200, 'OK'])
    end
